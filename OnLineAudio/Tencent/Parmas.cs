...
namespace Speech.Tencent
{
    ...
    public class Params
    {
        ...
        public int format = (int)Format.MP3;//  是 int 正整数 	2 	合成语音格式编码，定义见下文描述
        ...
        public int apc = 0;//    是 int

        public string ToParamsString()
        {
            var array = GetParams();
            var param = new string[array.Count];
            for (int i = 0; i < array.Count; i++)
            {
                var value = System.Uri.EscapeDataString(array[i].Value);
                param[i] = array[i].Key + "=" + value;
            }
        ...
        public List<KeyValuePair<string,string>> GetParams()
        {
            ...
            System.Array.Sort<System.Reflection.FieldInfo>(fields,Compire);
            for (int i = 0; i < fields.Length; i++) {
                pairs.Add(new KeyValuePair<string, string>(fields[i].Name, fields[i].GetValue(this).ToString()));
            }
            return pairs;
        }

        private int Compire(FieldInfo x, FieldInfo y)
        {
